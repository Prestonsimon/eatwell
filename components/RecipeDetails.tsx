...
import { 
  ...
  Check, Users, Plus, Minus, Bookmark // ✅ Added Bookmark here
} from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 animate-fadeIn">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <button 
          onClick={onBack}
          className="group flex items-center gap-2 text-stone-500 hover:text-emerald-600 font-semibold transition-colors"
        >
        ...
        <button 
          onClick={onSave}
          className={`flex items-center justify-center gap-2 px-6 py-2.5 rounded-full border shadow-sm font-bold transition-all active:scale-95 ${
            isSaved 
        ...
        >
          ...
          {isSaved ? 'Saved to Kitchen' : 'Save Recipe'}
        </button>
      </div>
             ...
             <div className="flex flex-wrap gap-6 md:gap-12 pt-8 border-t border-white/10">
                ...
                <div className="flex items-center gap-3">
                   ...
                   <div>
                     ...
                     <p className="font-semibold">{Math.round((recipe.calories || 0) * multiplier)} kcal</p>
                   </div>
                </div>
                <div className="flex items-center gap-3">
           ...
           <div className="md:col-span-4 bg-stone-50 p-8 border-r border-stone-100">
              ...
              <div className="bg-white border border-stone-200 rounded-2xl p-4 mb-6 shadow-sm">
                ...
                <div className="flex items-center justify-between">
                  <button onClick={() => adjustServings(-1)} className="w-8 h-8 rounded-full border border-stone-200 flex items-center justify-center text-stone-600 hover:border-emerald-500 hover:text-emerald-600 transition-all active:scale-95"><Minus size={16} /></button>
                  ...
                  <button onClick={() => adjustServings(1)} className="w-8 h-8 rounded-full border border-stone-200 flex items-center justify-center text-stone-600 hover:border-emerald-500 hover:text-emerald-600 transition-all active:scale-95"><Plus size={16} /></button>
                </div>
              </div>
              ...
              <button 
                ...
                className={`w-full mb-6 flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider border-2 transition-all duration-200 ${
                  copied ? 'bg-emerald-50 border-emerald-500 text-emerald-600' : 'bg-white border-stone-200 text-stone-600 hover:border-emerald-500 hover:text-emerald-600 shadow-sm'
                }`}
              >
                {copied ? (<><Check size={14} /><span>Copied!</span></>) : (<><Copy size={14} /><span>Copy Ingredients</span></>)}
              </button>
              ...
              <ul className="space-y-4">
                {(recipe?.ingredients || []).map((ingredient, idx) => (
                  ...
                  >
                    <div className={`mt-0.5 flex-shrink-0 w-5 h-5 rounded border-2 transition-all flex items-center justify-center ${checkedIngredients.has(idx) ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-stone-300 group-hover/item:border-emerald-400 bg-white'}`}>
              ...
              <div className="mt-8 bg-emerald-100/50 p-6 rounded-2xl border border-emerald-100">
                ...
                <p className="text-sm text-emerald-800/80 leading-relaxed italic">"{recipe.ecoTip}"</p>
              </div>
           </div>
              ...
              <div className="space-y-8">
                {(recipe?.instructions || []).map((step, idx) => (
                  <div key={idx} className="flex gap-4 group">
                    <div className="flex-shrink-0 w-10 h-10 rounded-full bg-stone-100 text-stone-900 font-bold flex items-center justify-center border-2 border-transparent group-hover:border-emerald-500 transition-all">
                      {idx + 1}
                    </div>
                    <p className="text-lg text-stone-800 leading-relaxed pt-1">{step}</p>
                  </div>
                ))}
              </div>
           </div>
